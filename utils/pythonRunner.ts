...
import { performanceMonitor, trackCodeExecution, trackTestExecution, trackError } from '@/lib/performance';
...
// Enhanced Pyodide loader with performance monitoring
...
async function loadPyodide() {
  ...
  isLoading = true;
  performanceMonitor.startTiming('pyodide_load');
  ...
  try {
    pyodideLoadPromise = (async () => {
      ...
      performanceMonitor.startTiming('packages_load');
      ...
      `);
      performanceMonitor.endTiming('packages_load');
...
def analyze_code(code):
    try:
      ...
      performanceMonitor.monitorPyodidePerformance(pyodide);
      performanceMonitor.recordMetric('pyodideStatus', 'ready');
      
      return pyodide;
    })();
    
    const result = await pyodideLoadPromise;
    performanceMonitor.endTiming('pyodide_load');
    isLoading = false;
    ...
    performanceMonitor.endTiming('pyodide_load');
    performanceMonitor.recordMetric('pyodideStatus', 'error');
    trackError('pyodide_load', error);
...
export async function runPythonTests(
  ...
  performanceMonitor.startTiming('test_execution');
...
run_with_timeout('''${userCode.replace(/'/g, "\\'")}''', ${options.timeout || 10.0})
    ...
    if (!codeExecutionResult.success) {
      trackError('code_execution', codeExecutionResult.error);
      
      const executionTime = Date.now() - startTime;
      performanceMonitor.endTiming('test_execution');
      trackTestExecution(executionTime);
    ...
    trackCodeExecution(codeExecutionResult.execution_time * 1000);
...
export async function runStaticAnalysis(
  ...
  try {
    ...
    trackError('static_analysis', error);
    for (const check of checks) {
      results.push({
        check,
        passed: false,
...
result
    ...
    trackError('visualization_execution', error);
    return {
      success: false,
      error: String(error)
    };
